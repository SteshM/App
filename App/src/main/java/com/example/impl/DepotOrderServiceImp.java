...
import com.example.utils.Exchanger;
...
public class DepotOrderServiceImp implements DepotOrderService {
    ...
    public ExtendedRes getOrderItems(Long depotOrderId){
        ...
        List<OrderItemDetails> orderItemDetails = depotOrderItems.stream().map(DepotOrderServiceImp::getOrderItemDetails).toList();
        return ExtendedRes.builder()
                .status(200)
    ...
    public MinimalRes assignDepotOrderToDriver(AssignDepotOrderToDriver assignDepotOrderToDriver) {
        ...
        Depot depot = depotRepository.findByDepotId(depotId);
        DepotDriver depotDriver = depotDriverRepo.findByDriverAndDepot(driver.get(), depot);
        if(depotDriver == null){
            return MinimalRes.builder()
                    .status(400)
    ...
    public ExtendedRes getDepotOrders( OrderStatus orderStatus) {
        ...
        List<DepotOrderRequest> depotOrderRequestDtos = depotOrders.stream().map(DepotOrderServiceImp::getDepotOrderRequest).toList();
        return ExtendedRes.builder()
                .status(200)
